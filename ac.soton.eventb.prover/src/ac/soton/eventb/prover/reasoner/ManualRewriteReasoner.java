...
import ac.soton.eventb.prover.rewriter.RuleBaseManualRewriter;
...
public class ManualRewriteReasoner implements IReasoner{
	...
	public static final String REASONER_ID = ProverPlugIn.PLUGIN_ID + ".ruleBaseManualReasoner";
	...
	public IReasonerInput deserializeInput(IReasonerInputReader reader)
		...
		if (length == 0) {
			// Goal rewriting
			return new Input(theoryString, ruleString, ruleDesc, null, position);
		}
		// Hypothesis rewriting
		if (length != 1) {
	...
	public void serializeInput(IReasonerInput input, IReasonerInputWriter writer)
		...
		writer.putString(THEORY_KEY, ((Input) input).theoryName);
		writer.putString(RULE_KEY, ((Input) input).ruleName);
		writer.putString(DESC_KEY, ((Input) input).ruleDesc);
	}
