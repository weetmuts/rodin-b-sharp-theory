...
public class TheoryInferenceRuleAttributesModule extends SCFilterModule {
	...
	public boolean accept(IRodinElement element, ISCStateRepository repository,
		...
		String toolTip = rule.getParent().getElementName() + "."
				+ rule.getLabel();
		...
		// check the tool tip
		if (!rule.hasToolTip()
				|| (rule.hasToolTip() && rule.getToolTip().equals(""))) {
		...
		if (rule.getInfers().length != 1) {
			createProblemMarker(rule, EventBAttributes.LABEL_ATTRIBUTE,
					TheoryGraphProblem.RuleRequireOneInfer, label);
			return false;
		}
		return true;
	}
	...
	private void checkAndSetSymbolInfo(String label, boolean isAuto,
			boolean isInter, String toolTip, String desc) {
