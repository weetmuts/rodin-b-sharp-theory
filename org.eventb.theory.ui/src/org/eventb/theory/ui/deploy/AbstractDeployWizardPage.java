...
public abstract class AbstractDeployWizardPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		new Label(container, SWT.NONE);
		{
			...
			dependButton.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER,
					false, false, 2, 1));
		...
		}
		{
			...
			forceButton.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false,
					false, 2, 1));
	...
	protected void populatePOs(IRodinFile file) {
		...
		try {
			...
			for (IPSStatus s : statuses) {
				...
				} else if (TheoryCoreFacade.isReviewed(s)) {
