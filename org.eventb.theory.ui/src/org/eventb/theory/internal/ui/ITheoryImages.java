/**
 * 
 */
package org.eventb.theory.internal.ui;

import org.eventb.ui.IEventBSharedImages;

/**
 * <p>Interface providing some useful static fields.</p>
 * @author maamria
 * 
 */
public interface ITheoryImages extends IEventBSharedImages{
	
	public static final String IMG_DATATYPE = "Datatype";
	public static final String IMG_DATATYPE_PATH = "icons/grd_obj.gif";
	
	public static final String IMG_RULES_BLOCK = "Rules Block";
	public static final String IMG_RULES_BLOCK_PATH = "icons/axms_obj.gif";
	
	public static final String IMG_TTHEOREM = "Theory Theorem";
	public static final String IMG_TTHEOREM_PATH = "icons/thm_obj.gif";
	
	public static final String IMG_OPERATOR = "Operator";
	public static final String IMG_OPERATOR_PATH = "icons/var_obj.gif";
	
	public static final String IMG_REWRITE_RULE = "Rewrite Rule";
	public static final String IMG_REWRITE_RULE_PATH = "icons/inv_obj.gif";
	
	public static final String IMG_RULE_RHS = "RHS";
	public static final String IMG_RULE_RHS_PATH = "icons/grd_obj.gif";
	
	public static final String IMG_TYPEPAR = "Type Parameter";
	public static final String IMG_TYPEPAR_PATH = "icons/cst_obj.gif";
	
	public static final String IMG_THEORY = "Theory";
	public static final String IMG_THEORY_PATH = "icons/thy.png";
	
	public static final String IMG_DTHEORY = "Deployed Theory";
	public static final String IMG_DTHEORY_PATH = "icons/thyDep.png";
	
	public static final String IMG_OTHEORY = "Outdated Theory";
	public static final String IMG_OTHEORY_PATH = "icons/thy-outdated.png";
}
