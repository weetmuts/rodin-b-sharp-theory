 ...
 *
 */
public class FComputer extends AbstractComputer {
	
	public FComputer(FormulaFactory formulaFactory) {
	...
	public void visitBinaryPredicate(BinaryPredicate predicate) {
		...
		switch (predicate.getTag()) {
		case Formula.LEQV:
			lemma = fb.lor(fb.land(new TComputer(fb.ff).getLemma(left), getLemma(right)), 
					fb.land(new TComputer(fb.ff).getLemma(right), getLemma(left)));
			break;
		case Formula.LIMP:
			lemma = fb.land(new TComputer(fb.ff).getLemma(left), getLemma(right));
			break;
		default:
			assert false;
			lemma = null;
	...
	public void visitUnaryPredicate(UnaryPredicate predicate) {
		lemma = new TComputer(fb.ff).getLemma(predicate.getChild());
	}
