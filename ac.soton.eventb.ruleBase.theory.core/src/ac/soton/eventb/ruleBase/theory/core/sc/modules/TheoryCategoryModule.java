 ...
 * <ul> 
 *  <li>if a category is specified more than once, then a warning is issued and duplicates are ignored.
 * 	<li>if a category is not pre-defined (in the theory preferences), an error is issued.
 * </ul>
 * @author maamria
 *
 */
public class TheoryCategoryModule extends SCProcessorModule {
	...
	public void process(IRodinElement element, IInternalElement target,
		...
		if(categories.length != 0){
			...
			for(ICategory cat : categories){
				...
				// check first that category is pre-defined
				if(TheoryUtils.contains(PrefsRepresentative.getCategories(), catName)){
					...
					if(!usedCategories.contains(catName)){
						...
						createProblemMarker(cat, TheoryAttributes.CATEGORY_ATTRIBUTE,
								TheoryGraphProblem.DuplicateCategoryWarning, catName);
					}
				}
				else {
					// issue an error, only allow pre-defined categories
					createProblemMarker(cat, TheoryAttributes.CATEGORY_ATTRIBUTE,
							TheoryGraphProblem.CategoryNotPredefined, catName);
				}
			}
		}
	}
	...
	throws RodinDBException{
		...
		scCat.setAttributeValue(TheoryAttributes.CATEGORY_ATTRIBUTE,categoryName, monitor);
	}
